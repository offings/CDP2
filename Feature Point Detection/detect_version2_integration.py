...
for i in range(33) :
    img = cv2.imread('./third_result/' + str(i+1) + '.jpg',cv2.IMREAD_COLOR)
    ...
    circles2 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 3, 10, param1=60, param2=50, minRadius=0, maxRadius=20)
    ...
    if (circles is not None) and (len(circles) < 5) :
        ...
        for (x,y,r) in circles2:
            cv2.circle(img, (x,y), r, (36,255,12), 5)
    cv2.imwrite('image/' + str(i+1) + '.jpg',img)
