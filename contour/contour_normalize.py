...
img = cv2.imread('../background_ver2/image/first_output/31.jpg')
out = cv2.imread('../background_ver2/image/second_output/31.jpg')
...
print("first")
for i in range(len(contour_info[4][0])) :
    x, y = contour_info[4][0][i][0]
    sum1 = sum1 + output[y][x]
    print(x, y, output[y][x])

print("second")
for j in range(len(contour_info[5][0])) :
    x, y = contour_info[5][0][j][0]
    sum2 = sum2 + output[y][x]
    print(x, y, output[y][x])

ave1 = sum1 / len(contour_info[4][0])
ave2 = sum2 / len(contour_info[5][0])
